...
struct Vertex {
	...
	// 為了unordered_map 中的比較操作重載運算符
	bool operator==(const Vertex& other) const {
		return pos == other.pos && color == other.color && texCoord == other.texCoord;
	}
};

// 為 hash 新增 Vertex 的哈希值計算方法
namespace std {
...
private:
	...
	void initWindow(){
		...
		if(!XMatchVisualInfo(display, screen, 32, TrueColor, &visual_info)){
			std::cerr << "No visual found supporting 32-bit color\n";
			exit(1);
		}
	...
	bool isDeviceSuitable(VkPhysicalDevice device){
		...
		std::cout << "內存塊最大可分配數量： " << deviceProperties.limits.maxMemoryAllocationCount << std::endl;
		
		VkPhysicalDeviceMemoryProperties memoryProperties;
		vkGetPhysicalDeviceMemoryProperties(device, &memoryProperties);
		std::cout << "顯存堆數量:\t" << memoryProperties.memoryHeapCount << std::endl;
		for(uint32_t i = 0; i < memoryProperties.memoryHeapCount; i++){
			std::cout << "Heap " << i << " : \t" << memoryProperties.memoryHeaps[i].size / (1024 * 1024);
			std::cout << "\tflags : \t" << memoryProperties.memoryHeaps[i].flags << std::endl;
		}
		
		
		std::cout << std::endl;
		std::cout << "type數量:\t" << memoryProperties.memoryTypeCount << std::endl;
		...
		std::cout << "anisotropy:\t" << deviceFeatures.samplerAnisotropy << std::endl;
	...
	void createTextureImage(){
		...
		//transitionImageLayout(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, mipLevels); // 轉變移到generateMipmaps中處理
	...
	void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels){
		...
		barrier.image = image;
		//barrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT; 變成參數在下面做選擇了
	...
	void createTextureSampler(){
		...
		if(vkCreateSampler(device, &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS){
			throw std::runtime_error("dailed to create texture sampler!");
		}
	}
